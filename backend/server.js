...
const app = express();
app.use(cors({
    origin: ["https://aethercraft-silk.vercel.app", "http://localhost:5173"],
        ...
        if (await recipeExists(element1Id, element2Id)) {                                             // not a new recipe
            ...
            if (!aiResult) {
                ...
                if (isDuplicate) {
                    ...
                    try {
                        const imageBuffer = await generateImage(ename);
                        if (imageBuffer) {
                            const filename = `${ename.replace(/\s+/g, '_')}_${Date.now()}`;
                            permanentUrl = await addToImages(imageBuffer, filename);
    ...
    try {
        ...
        if (!inventory || inventory.length === 0) {
            ...
            if (baseElements && baseElements.length > 0) {
                ...
                if (insertError) {
                    console.error("❌ Failed to gift starter pack to inventory:", insertError);
                } else {
